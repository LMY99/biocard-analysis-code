...
nX <- ncol(X)
sigmays <- rep(0, R) # Error variance
sigmaws <- rep(0, R) # Random effect variance
pens <- array(0, c(2, R)) # Penalty parameter
REs <- array(0, c(dim(long_ss), R)) # Random effects
offsets <- array(0, c(dim(long_ss), R)) # Random effects + Covariate shift
# Sample a initial value for parameters
coefs[1:nX, , 1] <-
  t(rtmvnorm(ncol(Y),
    mu = beta.prior$mean,
...
# Prior log-density for penalties
lpd <- function(s) {
...
lss <- ls # Sequence of LS in adaptive MCMC
